...
void ISocketServer::listenThreadFunction()
{
    ...
    while(isListening){
        ...
        if(hasMsg == true){
            __android_log_print(ANDROID_LOG_ERROR, "vnc_Socket_Server", "RECEIVED MSG FROM CLIENT: %s\n", messageBuffer);
...
bool ISocketServer::connect(int port)
{
    ...
    if(sockfd < 0){
        __android_log_print(ANDROID_LOG_ERROR, "VNC_ISocketServer", "ERROR opening socket");
    ...
    if (bind(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0){
        __android_log_print(ANDROID_LOG_ERROR, "VNC_ISocketServer", "ERROR on binding");
        CommonFuncs::error("ERROR on binding");
        return false;
    }
    
    __android_log_print(ANDROID_LOG_DEBUG, "VNC_ISocketServer", "Start listening for connection .. .. .. ");
